...
def get_current_price():
    sources = [
        ...
        },
        {
            ...
            "parser": lambda data: float(data["result"])
        },
        {
            "name": "Alpha Vantage",
            "url": "https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=XAU&to_currency=USD&apikey=demo",
            "parser": lambda data: float(data["Realtime Currency Exchange Rate"]["5. Exchange Rate"])
        }
    ]
...
def get_metals_api_klines(limit=200):
    url = "https://metals-api.com/api/timeseries"
    params = {
        "access_key": "demo",
        "symbols": "XAU",
        "start_date": (datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=30)).strftime("%Y-%m-%d"),
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    candles = get_metals_api_klines(limit=200)
    if not candles:
