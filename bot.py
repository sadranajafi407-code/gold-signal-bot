...
def parse_tradermade_csv(file_content):
    try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "دستورات:\n"
        "/signal - دریافت راهنمای دانلود داده و ارسال فایل CSV\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 راهنمای ربات\n\n"
        "1️⃣ ابتدا دستور /signal را بزنید.\n"
        "2️⃣ از لینک داده شده، فایل CSV آخرین داده‌های طلا را دانلود کنید.\n"
        "3️⃣ فایل را برای ربات ارسال کنید (به‌عنوان سند).\n"
        "4️⃣ ربات داده‌ها را تحلیل کرده و سیگنال را به شما نمایش می‌دهد.\n\n"
        "🔹 /signal : دریافت لینک دانلود داده\n"
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "📥 **مراحل دریافت سیگنال:**\n\n"
        "۱. روی لینک زیر کلیک کنید تا آخرین داده‌های قیمت طلا را دریافت کنید:\n"
        "🔗 [دانلود داده‌های XAUUSD از Tradermade](https://tradermade.com/download/forex/XAUUSD-1H.csv)\n\n"
        "۲. فایل CSV را دانلود کرده و آن را به‌عنوان **سند (Document)** برای من ارسال کنید.\n"
        "۳. من داده‌ها را تحلیل کرده و سیگنال خرید یا فروش را به شما نشان می‌دهم.\n\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        candles = parse_tradermade_csv(file_content)
