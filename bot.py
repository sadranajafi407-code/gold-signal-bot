...
def get_current_price():
    try:
        ...
        pass
    try:
        resp = requests.get("https://api.exchangerate.host/convert?from=XAU&to=USD", timeout=15)
        if resp.status_code == 200:
            data = resp.json()
...
def load_metadata():
    if os.path.exists(METADATA_FILE):
        with open(METADATA_FILE, "r") as f:
            return json.load(f)
    return {"last_date": None, "row_count": 0}

def save_metadata(metadata):
    with open(METADATA_FILE, "w") as f:
        json.dump(metadata, f, indent=2)

def parse_csv_to_candles(file_content):
    try:
        data = csv.reader(io.StringIO(file_content.decode('utf-8')))
        candles = []
        next(data, None)
        for row in data:
            if len(row) >= 6:
                try:
                    ...
                    candles.append({
                        ...
                        "low": float(row[3]),
                        "close": float(row[4]),
                        "volume": float(row[5]) if row[5] else 1000,
                        "date": dt
                    })
                except:
                    continue
...
def save_candles_to_csv(candles, filepath):
    with open(filepath, "w", newline='') as f:
        ...
        for c in candles:
            date_str = datetime.fromtimestamp(c["time"]).strftime("%Y-%m-%d %H:%M:%S")
            writer.writerow([date_str, c["open"], c["high"], c["low"], c["close"], c["volume"]])
...
def append_candles_to_csv(new_candles, filepath):
    existing_candles = load_candles_from_csv(filepath)
    ...
    save_candles_to_csv(unique_candles, filepath)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/update_data - به‌روزرسانی داده‌ها با فایل CSV جدید\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 راهنمای ربات\n\n"
        "1️⃣ ابتدا یک فایل CSV پایه (حداقل ۲۰ روز داده) را با دستور /update_data برای ربات ارسال کنید.\n"
        "2️⃣ پس از ذخیره‌سازی داده‌ها، هر بار با دستور /signal سیگنال جدید دریافت کنید.\n"
        "3️⃣ برای به‌روزرسانی داده‌ها، فایل CSV جدید را با /update_data ارسال کنید.\n\n"
        "🔹 /signal : دریافت سیگنال\n"
        "🔹 /update_data : ارسال فایل CSV جدید برای به‌روزرسانی داده‌ها\n"
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not os.path.exists(DATA_FILE):
        await update.message.reply_text(
            ...
            "لطفاً ابتدا با دستور /update_data یک فایل CSV پایه (حداقل ۲۰ روز داده) ارسال کنید."
        )
        return
    ...
    if len(candles) < 20:
        await update.message.reply_text(
            "⚠️ داده‌های کافی در فایل وجود ندارد (حداقل ۲۰ کندل نیاز است).\n"
            "لطفاً با /update_data یک فایل CSV معتبر ارسال کنید."
        )
        return
    ...
    if not price:
        ...
        await update.message.reply_text("⚠️ قیمت لحظه‌ای دریافت نشد، از قیمت آخرین کندل استفاده می‌شود.")
    ...
    msg = (
        ...
        f"📊 تایم‌فریم‌ها:\n" + "\n".join(tf_lines) + "\n\n"
        f"💰 قیمت ورود: {price}\n"
        f"📦 حجم پیشنهادی: {risk['lot_size']} لات\n"
        f"🎯 ورود: {price}\n\n"
        + risk_line + "\n"
        + fib_line
        + prev_line
...
async def update_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📥 **مراحل به‌روزرسانی داده‌ها:**\n\n"
        "۱. یک فایل CSV با داده‌های جدید (حداقل ۲۰ کندل) آماده کنید.\n"
        "۲. فایل را به‌عنوان **سند (Document)** برای من ارسال کنید.\n"
        "۳. من داده‌ها را به داده‌های موجود اضافه می‌کنم.\n\n"
        "⚠️ **توجه:** فایل باید فرمت CSV با ستون‌های `Date,Open,High,Low,Close,Volume` داشته باشد."
    )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not document.file_name.endswith('.csv'):
        await update.message.reply_text("❌ لطفاً فقط فایل CSV ارسال کنید.")
        return
    try:
        file = await document.get_file()
        file_content = await file.download_as_bytearray()
        new_candles = parse_csv_to_candles(file_content)
        if not new_candles or len(new_candles) < 20:
            await update.message.reply_text("❌ فایل CSV نامعتبر یا داده‌های کافی در آن وجود ندارد. حداقل ۲۰ کندل نیاز است.")
            return
        
        all_candles = append_candles_to_csv(new_candles, DATA_FILE)
        
        metadata = load_metadata()
        if all_candles:
            metadata["last_date"] = datetime.fromtimestamp(all_candles[-1]["time"]).strftime("%Y-%m-%d %H:%M:%S")
            metadata["row_count"] = len(all_candles)
            save_metadata(metadata)
        
        await update.message.reply_text(
            f"✅ داده‌ها با موفقیت به‌روزرسانی شدند.\n"
            f"📊 تعداد کل کندل‌ها: {len(all_candles)}\n"
            f"📅 آخرین تاریخ داده: {metadata['last_date']}\n\n"
            f"اکنون می‌توانید با دستور /signal سیگنال جدید دریافت کنید."
        )
    except Exception as e:
...
def main():
    ...
    app.add_handler(CommandHandler("update_data", update_data))
