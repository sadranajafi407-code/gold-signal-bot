...
def get_data_from_twelvedata(symbol="XAU/USD", interval="1h", limit=300):
...
def get_market_data():
    candles = get_data_from_twelvedata(limit=300)
    if candles and len(candles) >= 20:
        return candles
    candles = get_data_from_gold_api(limit=500)
    if candles and len(candles) >= 20:
        return candles
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 راهنمای ربات\n\n"
        "این ربات داده‌های طلا را از منابع مختلف دریافت می‌کند.\n"
