...
# ======================== دریافت داده‌های کندلی (چند منبع) ========================
def get_data_from_mt5(symbol="XAUUSD", timeframe=mt5.TIMEFRAME_H1 if MT5_AVAILABLE else None, count=500):
    ...
    try:
        ...
        for rate in rates:
            candles.append({
                ...
                "low": rate[3],
                "close": rate[4],
                "volume": rate[5] if rate[5] else 1000
            })
        return candles
    except:
        return None

def get_data_from_yfinance(symbol="XAUUSD=X", interval="1h", period="1mo"):
    if not YF_AVAILABLE:
        return None
    try:
...
def get_data_from_gold_api(limit=500):
    try:
...
def get_market_data():
    # اولویت ۱: متاتریدر (ویندوز)
    candles = get_data_from_mt5()
    if candles and len(candles) >= 20:
        return candles
    # اولویت ۲: yfinance (لینوکس/مک)
    candles = get_data_from_yfinance()
    if candles and len(candles) >= 20:
        return candles
    # اولویت ۳: Gold-API
    candles = get_data_from_gold_api()
    if candles and len(candles) >= 20:
        return candles
    return None
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 راهنمای ربات\n\n"
        "این ربات به‌صورت خودکار داده‌های طلا را از منابع مختلف دریافت می‌کند.\n"
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        # دریافت داده از منابع خودکار
        candles = get_market_data()
        if not candles:
            await update.message.reply_text(
                "❌ خطا در دریافت داده از تمام منابع.\n"
                "لطفاً مطمئن شوید اینترنت متصل است و دوباره تلاش کنید."
            )
            return
        ...
        fear_greed = 50  # می‌توانید از get_fear_greed_index() استفاده کنید اگر در دسترس است
