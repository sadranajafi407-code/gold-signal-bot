...
def get_gold_api_klines(limit=200):
    url = "https://api.gold-api.com/price/XAU/history"
    params = {"limit": limit}
    try:
        ...
        if "prices" in data and data["prices"]:
            ...
            for item in data["prices"]:
                try:
                    date_str = item["date"].replace("Z", "")
                    candles.append({
                        "time": int(datetime.fromisoformat(date_str).timestamp()),
...
def get_twelvedata_klines(symbol="XAU/USD", interval="1h", limit=200):
...
def get_yahoo_klines(symbol="XAUUSD=X", interval="1h", limit=200):
    url = f"https://query1.finance.yahoo.com/v8/finance/chart/{symbol}"
    params = {"interval": interval, "range": "7d", "includePrePost": "false"}
    try:
        resp = requests.get(url, params=params, timeout=15)
        data = resp.json()
        ...
        for i in range(len(timestamps)):
            if ohlc["high"][i] is not None and ohlc["high"][i] > 0:
                candles.append({
                    "time": timestamps[i],
                    ...
                    "volume": float(ohlc["volume"][i]) if ohlc["volume"][i] else 1000
                })
        if candles:
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not price:
            await update.message.reply_text("❌ خطا در دریافت قیمت. لطفاً چند دقیقه دیگر تلاش کنید.")
            return
        
        candles = get_gold_api_klines(limit=200)
        if not candles:
            candles = get_twelvedata_klines(limit=200)
        if not candles:
            candles = get_yahoo_klines(limit=200)
        if not candles:
            await update.message.reply_text("❌ خطا در دریافت داده‌های بازار. هیچ منبع داده‌ای در دسترس نیست.")
            return
        ...
        error_message = f"❌ خطای داخلی در پردازش سیگنال:\n`{str(e)}`"
