...
# ======================== دریافت قیمت با ۳ منبع ========================
def get_current_price():
    sources = [
        {
            "name": "KuCoin",
            "url": "https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=XAU-USDT",
        ...
        },
        {
            ...
            "parser": lambda data: float(data["chart"]["result"][0]["meta"]["regularMarketPrice"])
        }
    ]
    ...
    for source in sources:
        try:
            print(f"⏳ تلاش برای دریافت قیمت از {source['name']}...")
            resp = requests.get(source["url"], headers=headers, timeout=15)
            print(f"📡 وضعیت پاسخ از {source['name']}: {resp.status_code}")
            
            if resp.status_code == 200:
                data = resp.json()
                price = source["parser"](data)
                if price and price > 0:
                    print(f"✅ قیمت از {source['name']} دریافت شد: {price}")
                    return price
            else:
                print(f"⚠️ پاسخ ناموفق از {source['name']}: {resp.status_code}")
        except Exception as e:
            print(f"❌ خطا در {source['name']}: {str(e)}")
            continue
...
async def signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not candles:
        await update.message.reply_text("❌ خطا در دریافت داده‌های بازار. لطفاً چند دقیقه دیگر تلاش کنید.")
        return
